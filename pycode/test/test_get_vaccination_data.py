...
class TestGetVaccinationData(unittest.TestCase):
    ...
    def test_get_mocked_vaccination_data_no_errors(self, mock_vaccination):
        getVaccinationData.main()

    def test_get_standart_vaccination_data_no_errors_with_plots(self):
        getVaccinationData.get_vaccination_data(make_plot=True)
    ...
    @patch('epidemiology.epidata.getVaccinationData.pd.read_csv',side_effect=ImportError())
    ...
    def test_split_column_based_on_values(self):
        ...
        df_split = getVaccinationData.split_column_based_on_values(df_to_split,column_ident,column_vals_name,new_col_labels) 
        ...
        for i in range(0,len(indent_unique)):
            number_unique_idents_df.append(len(df_to_split[df_to_split[column_ident] == indent_unique[i]]))

        self.assertEqual(len(df_split),3, 'Did not divide dataframe in three sperate ones.')
        for i in range(0,len(new_col_labels)):
            self.assertEqual(number_unique_idents_df[i],len(df_split[i]),'Length of Dataframe is wrong.')
            column_names_i = df_split[i].columns
            for j in range(0,len(column_names_i)):
                self.assertEqual(new_column_names[i][j],column_names_i[j],'Column names do not match.')
    ...
    def test_split_column_based_on_values_ident_length_wrong(self):
        ...
        column_ident = 'Impfschutz'
        column_vals_name = 'Anzahl'
        new_col_labels = ['Vacc_partially', 'Vacc_completed'] #wrong amount for data
        df_split = getVaccinationData.split_column_based_on_values(df_to_split,column_ident,column_vals_name,new_col_labels) 
    ...
    def test_intervall_mapping(self):
        lower_bounds1 = np.array([0, 3, 6, 15, 18, 25, 30, 40, 50, 65, 74, 100])
        upper_bounds1 = np.array([0, 3, 5, 6, 12, 15, 18, 25, 30, 35, 40, 50, 60, 65, 74, 80, 100])
        ...
        map_bounds1 = getVaccinationData.create_intervals_mapping(
            lower_bounds1, upper_bounds1)
        map_bounds2 = getVaccinationData.create_intervals_mapping(
