...
class TestGetDiviData(fake_filesystem_unittest.TestCase):
    ...
    def test_gdd_download_data_for_one_day(self, mock_ccu):
        ...
        for test_date in self.test_url_ending_else.keys():
            ...
            [call_number, df, call_string] = gdd.download_data_for_one_day(
                self.test_url_ending_else[test_date][1] - 1, test_date)
            self.assertTrue(
        ...
        expected_calls = [
            call.call_call_url(test_date_str, test_call_number - 3),
    ...
    def test_gdd_download_data_for_one_day_error(self, mock_ccu):
        ...
        exit_string = "Something went wrong with download of data for date " + \
            str(download_date) + ", although it is part of the call_number_dict."
    ...
    def test_gdd_read_data(self):
        ...
        with open(file_with_path, 'w') as f:
            f.write(self.test_string_read_fulldata.replace(
                "faelle_covid_aktuell_beatmet",
                "faelle_covid_aktuell_invasiv_beatmet"))
    ...
    def test_gdd_update_data(self, mock_loadcsv, mock_print, mock_ddfod):
        ...
        gdd.get_divi_data(
            read_data, file_format, out_folder, no_raw, end_date=date.today(),
            start_date=dd.defaultDict['start_date'],
            update_data=True)
