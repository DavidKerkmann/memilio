...
class Test_modifyDataframeSeries(fake_filesystem_unittest.TestCase):
    ...
    test_df1 = pd.DataFrame(
        {'Date': [datetime(2021, 1, 5), datetime(2021, 1, 6), datetime(2021, 1, 7), datetime(2021, 1, 8)],
         'test_col1': [11.0, 3.0, 3.0, 6.0],
         'test_col2': ['a', 'x', 'x', 't']})
    test_df2 = pd.DataFrame(
        {'Date': [datetime(2021,1,4), datetime(2021, 1, 5), datetime(2021, 1, 6), datetime(2021, 1, 7), datetime(2021, 1, 8)],
         'test_col1': [11.0, 11.0, 3.0, 3.0, 6.0],
         'test_col2': ['a', 'a', 'x', 'x', 't']})
    test_df2 = pd.DataFrame(
        {'Date': [datetime(2021,1,4), datetime(2021, 1, 5), datetime(2021, 1, 6), datetime(2021, 1, 7), datetime(2021, 1, 8)],
         'test_col1': [0.0, 11.0, 3.0, 3.0, 6.0],
         'test_col2': ['', 'a', 'x', 'x', 't']})
    def setUp(self):
        self.setUpPyfakefs()
    ...
    def test_impute_and_reduce_df(self):
        ...
        test_df_new = mDfS.impute_and_reduce_df(
            ...
            moving_average=0, min_date='2021-01-05', max_date='2021-01-08', start_w_firstval=False)
        pd_testing.assert_frame_equal(test_df_new,self.test_df1)
        
        # test start_w_firstval
        test_df_new = mDfS.impute_and_reduce_df(
            self.filldates_df, group_by_cols, mod_cols, impute='forward',
            moving_average=0, min_date='2021-01-04', max_date='2021-01-08', start_w_firstval=True)
        pd_testing.assert_frame_equal(test_df_new,self.test_df2)
        test_df_new = mDfS.impute_and_reduce_df(
            self.filldates_df, group_by_cols, mod_cols, impute='forward',
            moving_average=0, min_date='2021-01-04', max_date='2021-01-08', start_w_firstval=False)
